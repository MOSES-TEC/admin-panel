  ...
  export default async function handler(req, res) {
    try {
      ...
      if (method === "GET") {
        ...
        if (id) {
          let query = Developer.findById(id);
          const refFields = ['users'];
          if (refFields.length) {
      ...
      if (method === "POST") {
        ...
        const doc = await Developer.create({ icon, seller, Date, users, user, seoTitle, seoDescription, focusKeywords, canonicalUrl, metaRobots, openGraphTitle, openGraphDescription });
        return res.json(doc);
      }
      ...
      if (method === "PUT") {
        ...
        await Developer.updateOne({ _id }, { icon, seller, Date, users, user, seoTitle, seoDescription, focusKeywords, canonicalUrl, metaRobots, openGraphTitle, openGraphDescription });
